...
import SupervisionTraineeTag from "./supervision-trainee-tag";
...
const SupervisionTraineeSelector: FC<Props> = ({
  id = "",
  ...
  return (
    <>
      <div className="supervision-trainee-selector">
        ...
        <Card className="tag-viewport" size="small">
          {Array.from(value.values()).map((m) => (
            <SupervisionTraineeTag key={m.id} member={m} deletable onDelete={onDelete} />
          ))}
        </Card>
      </div>
    </>
...
export default SupervisionTraineeSelector;
